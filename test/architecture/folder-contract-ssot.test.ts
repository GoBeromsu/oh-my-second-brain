...
const MODEL_CONTEXT_READERS = [
  new URL(
    "../../src/kernel/engine/retrieval/folder-context.ts",
    import.meta.url,
  ),
] as const;
...
describe("sealed folder meaning is the sole model-context source", () => {
  it("keeps model context on sealed folder meaning alone", async () => {
    ...
    for (const reader of MODEL_CONTEXT_READERS) {
      ...
      expect(source).toContain("resolveSealState");
      expect(source).toContain('FOLDER_CONTEXT_SOURCE = "folders.json"');
      expect(source).not.toMatch(/\.oms\//u);
  ...
  it("routes both expansion and reranking through the folder projection", async () => {
    const source = await readFile(FACADE, "utf8");

    expect(source).toContain("loadFolderIntentProjection(");
    expect(source).toContain("context: context.promptContext");
    expect(source).toContain("Vault folder intents:");
    expect(source).toContain("folderIntents: folderProjection?.matched");
  });
