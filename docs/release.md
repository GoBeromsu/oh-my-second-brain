...
5. **CI-only, tokenless publish.** `npm publish` runs inside `.github/workflows/release.yml` and nowhere else. There is no local publish path, and the operator script never invokes npm publish. Authentication is npm [trusted publishing](https://docs.npmjs.com/trusted-publishers) over GitHub OIDC: the registry trusts this repository and that exact workflow filename, so the job carries no npm token and no repository secret needs rotating. The package's publishing policy also forbids 2FA-bypass tokens, which makes OIDC the only accepted path rather than merely the preferred one.
