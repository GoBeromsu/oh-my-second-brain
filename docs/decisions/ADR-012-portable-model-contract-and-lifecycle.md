...
This boundary also governs embedding compatibility. ADR-007 requires real,
native-dimension embeddings and forbids fake production fallbacks. A change to
model revision, weights, checksum, or prompt formatting changes embedding
meaning even when a model name remains the same. ADR-010 keeps the in-repo
engine as the default search backend, and ADR-011 keeps reranking explicit
opt-in until a real production reranker is assembled.
...
any provider authentication variables. A malformed or incomplete higher tier
is an error and never falls through to a lower tier.
...
- A benchmark gate is deferred to a later approved milestone.
- The B2 generator/context work is deferred to a later approved milestone.
- This ADR makes no claim that qmd has been replaced.
