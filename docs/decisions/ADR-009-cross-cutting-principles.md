...
- 구 ADR-0001은 `validateFrontmatter`가 `ValidationResult`를 반환하고 doctor는 항상 exit 0이라고 했지만, 구현에는 `validateFrontmatter`가 없다. `oms doctor`는 은퇴했고 exit 1과 대체 안내를 낸다(`src/cli/oms.ts:34-47,210-218`). 검사 명령은 finding이 있으면 exit 1이다(`src/cli/contract-command.ts:166-169`, `src/cli/link-command.ts:238`).
...
- 검사와 판정 함수는 결과 값을 반환한다. 예: `judge`는 throw하지 않고 `{ok, violations, missingDefaults}`를 반환하며, 각 위반은 `{field, kind}`뿐이다(`src/kernel/contract/judge.ts:10-13,246-260`).
- reader는 diagnostic을 반환한다(`src/kernel/engine/retrieval/template-source.ts`, `src/kernel/conventions/note-exclude.ts`).
- 상태 조회는 오류가 나도 null status를 반환한다(`src/kernel/engine/mcp/facade.ts:1123-1134`).
...
- 검색 경로의 오류는 `queryResultUnavailable`로 반환한다(`facade.ts:707`).
- 봉인이 없으면 `CONTRACT_OPEN`, 봉인된 계약을 읽을 수 없으면 `CONTRACT_UNREADABLE`을 보고한다(`src/kernel/engine/retrieval/template-source.ts:113,122`).
...
- 검색 쪽 계약 reader는 vault를 인정하거나 쓰지 않고, `settings.json` 외의 볼트 제어 파일을 읽지 않는다(`src/kernel/engine/retrieval/template-source.ts:8-16`).
- 본문 lazy-load는 파생 상태를 만들지 않는다(`src/kernel/graph/cache.ts:13`).
- 그래프 cache는 explicit build만 저장한다(→ ADR-006 §6).
- 폴더 문맥은 봉인된 `folders.json`만 출처로 삼는다(`src/kernel/engine/retrieval/folder-context.ts:5-6`, `test/architecture/folder-contract-ssot.test.ts`).

### 5. 결정적 순서

사용자에게 보이는 목록과 digest 입력은 locale과 무관한 code-point 순서로 정렬한다. 근거:
- `src/kernel/conventions/note-exclude.ts:68`
- `src/kernel/contract/interview.ts:199,247,264`
- `src/kernel/contract/status.ts:59,96`
- `src/kernel/contract/judge.ts:180,193,222`
- `src/kernel/engine/retrieval/template-source.ts:60`
- `src/kernel/engine/retrieval/folder-context.ts:24`

### 6. Obsidian 타입은 읽기 전용 관측

`.obsidian/types.json`은 단일 read-only reader로만 읽고, oms는 쓰지 않는다(`src/kernel/contract/obsidian.ts:34-37`). 이 정보를 계약에 어떻게 반영하는지는 → ADR-007이 소유한다.

### 7. 의미 판정은 agent·사용자의 몫

코드는 봉인된 구조만 판정한다. 허용된 쓰기는 노트가 구조에 맞는다는 뜻이지 보존할 가치가 있다는 뜻이 아니며, 그 판단은 agent와 사용자에게 남는다(`src/kernel/contract/judge.ts:10-13`). note body 수정도 oms가 하지 않는다. 예를 들어 `linkify`는 은퇴했다(`src/cli/oms.ts:39`).

### 8. 은퇴한 경로는 거부한다, alias하지 않는다

은퇴한 명령은 대체 명령을 안내하고 exit 1로 끝난다. alias로 동작을 유지하지 않는다(`src/cli/oms.ts:34-47,210-218`). 은퇴한 hook leaf도 같은 방식으로 거부한다(`oms.ts:138`). 은퇴한 볼트 파일(taxonomy.yaml, `.oms/`의 settings.json 외 항목)은 이전하지 않고 무시하며, `oms contract doctor`가 예상하지 않은 제어 파일로 보고한다(`src/kernel/contract/status.ts:86-100`, → ADR-007).
