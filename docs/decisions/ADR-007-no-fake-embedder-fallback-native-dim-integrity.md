...
> **Corrected on 2026-08-24:** The former `OMS_MODEL_PATH` error-contract description was inaccurate. With no embedding provider configured, a `vec` sub-query returns `available: false` and the reason: `"graph-only engine: embedding provider unavailable. Configure embeddings via OMS_EMBEDDING_PROVIDER + OMS_EMBEDDING_MODEL (and provider auth env vars, e.g. UPSTAGE_API_KEY) for engine semantic ops."` The two principles below are unchanged.
...
- provider가 설정되지 않았을 때 vec sub-query는 `available: false`와 `OMS_EMBEDDING_PROVIDER` + `OMS_EMBEDDING_MODEL` 및 provider 인증 환경변수 안내를 반환한다. 가짜 임베더로 묵시적 대체는 없다.
...
- provider가 없으면 graph-only 엔진은 계속 사용할 수 있고 vec sub-query는 `available: false`를 반환한다. semantic ops에는 `OMS_EMBEDDING_PROVIDER` + `OMS_EMBEDDING_MODEL` 및 해당 provider 인증 환경변수가 필요하다.
