...
describe("inspectInstalledAssets", () => {
  ...
  it("reports registration evidence without inspecting its declared path", async () => { expect((await inspectInstalledAssets({ assets: [{ id: "registration", kind: "registration", declaredPath: "/unread", evidence: { state: "ok", cause: null } }] })).assets[0]).toMatchObject({ state: "ok", realPath: null }); });
