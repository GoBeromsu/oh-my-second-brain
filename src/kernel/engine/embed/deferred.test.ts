...
describe("deferred graph-only embedding primitives", () => {
  ...
  it("does not claim a graph-only engine, which most of its callers are not", async () => {
    ...
    // user running `oms semantic vsearch` on an ordinary model-less vault was told
  ...
  it("tells the user every way to configure embeddings, not just the env pair", async () => {
    ...
    // A remote provider needs its own credential; dropping this would leave an
    // Upstage user with a complete-looking pair and an unexplained failure.
    expect(reason).toMatch(/UPSTAGE_API_KEY/);
