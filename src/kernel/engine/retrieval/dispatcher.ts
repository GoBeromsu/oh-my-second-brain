...
export interface DispatcherDeps {
   ...
   * An explicit graph sub-query fails when this is absent.
   */
...
async function dispatchOne(
  ...
  switch (sub.type) {
    ...
    case "graph": {
      if (!deps.graphTraverse) {
        throw new Error(
          "Explicit graph retrieval requires a configured graph traversal backend. " +
            "Assemble an engine with graph traversal support before issuing graph sub-queries.",
        );
      }
...
export async function dispatch(
  ...
  // RRF keeps metadata from the first list that contributes a key. Hydrate text
  // from any matching store result so a content-bearing candidate is not lost
  // merely because another modality ranked the same chunk first.
  const textByHitKey = new Map<string, string>();
  for (const list of policyLists) {
    for (const hit of list) {
      const key = `${hit.docPath}\x00${hit.chunkOrdinal}`;
      const text = hit.text ?? deps.store.getChunkText?.(hit.docPath, hit.chunkOrdinal);
      if (typeof text === "string" && text.trim() !== "") {
        if (!textByHitKey.has(key)) textByHitKey.set(key, text);
      }
    }
  }
