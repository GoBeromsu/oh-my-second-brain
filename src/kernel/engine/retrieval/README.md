...
- **`graph`** needs an injected `graphTraverse`. The legacy tracer wires one; the assembled engine does not, and an unwired `graph` sub-query currently yields an empty list rather than an explicit refusal. That asymmetry with the other three is a known gap, not a design choice.
