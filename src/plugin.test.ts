...
describe("codex and hermes adapter DoD", () => {
  it("Codex adapter ships native rule, MCP descriptor, and namespaced skills", async () => {
    ...
    for (const name of ["oms-setup", "oms-install", "oms-uninstall", "oms-doctor", "oms-capture", "oms-write", "oms-retrieve", "oms-update"]) {
  ...
  it("Hermes adapter ships a local skill bundle for install", async () => {
    ...
    for (const name of ["setup", "install", "uninstall", "doctor", "capture", "write", "retrieve", "update"]) {
