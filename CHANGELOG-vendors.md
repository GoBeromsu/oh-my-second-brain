...
- **The remaining Codex retry-budget requirements are gone.** The installed rule and the convention shim still bounded repair by the user's finite retry budget, which version 5 neither declares nor counts. Both now name `.oms/settings.json` as the repair authority and tell the agent to ask rather than retry blindly.
