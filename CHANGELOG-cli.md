...
- **`oms setup` inspects a vault before it proposes anything and refuses to overwrite history.** An actual version 5 vault is reported as configured with or without a native marker and with or without settings, a historical publication marker holds setup instead of replacing it, a native in-progress or rolling-back marker blocks setup until it is recovered, and a rolled-back schema migration is retained as its own retry anchor. Preparation is read-only: identity mode mismatches, unverified targets, unsafe control paths, and malformed global registries are reported as blockers without creating `.oms`.
- **Setup publishes portable settings and an explicit model configuration as separate, recoverable steps.** The vault UUID and template roots are published through the bounded publication protocol, the model configuration is written only into an absent `.oms/models.json` through an exclusive owned staging file, and a changed token, source, digest, or model choice is refused before any effect. Repeating the same token returns the original native receipt instead of publishing twice.
...
- The top-level qmd-compatible aliases `oms query`, `oms vsearch`, `oms get`, `oms multi-get` and `oms status` are gone, with 구 ADR-009 D2 (now ADR-001). The canonical nested commands — `oms semantic query|status|get|multi-get|vsearch` — are unaffected and remain the supported surface.
