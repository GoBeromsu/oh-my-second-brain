...
- **Retrieval reports an unreadable control instead of failing the route.** A policy or taxonomy that exists but cannot be read now yields unavailable metadata with `TEMPLATE_POLICY_UNREADABLE` or `TEMPLATE_TAXONOMY_UNREADABLE`, so search stays available with a named reason rather than raising a generic tool error. `search { op: "template-scan" }` answers from one contract read, so its revision and registrations cannot disagree, and the `write` tool description no longer promises the retired completion step.
- **The advertised tool schema matches the implemented routes.** The `write` tool's template branches are `publish-contract`, `review-sources`, `acknowledge-source`, and `relink-source`, each requiring its own evidence and an explicit `transactionId`; the retired interview-ledger branches, the transaction resume branch, and the `dryRun`/`approvedDigest` pair are gone, and `doctor` no longer advertises `regenerate-types`. A client reading `tools/list` can no longer construct a call the server refuses.
...
- `oms_search` advertises `limit` (default 10), `minScore` (default 0) and `rerank` (default false). The first two are applied at the normalizer, so an options-free query is bounded rather than unlimited. `rerank` is opt-in per 구 ADR-011 (now ADR-004): `true` reranks only when startup was given a real reranker and otherwise fails loudly with configuration guidance, rather than silently returning unreranked results as if the request had been honoured.
...
- The qmd-compatible aliases `query`, `get`, `multi_get` and `status`, and the `qmd://` resource, are gone. 구 ADR-009 D2 was superseded by 구 ADR-010; D1 remains in force (both now ADR-001/ADR-004).
